...
func CreateAndAttachVolume(client IncusClient, pool, volumeName, instanceName, deviceName, path string, sizeGB int) error {
	if err := client.CreateVolume(pool, volumeName, sizeGB); err != nil {
		return fmt.Errorf("create volume %s: %w", volumeName, err)
	}

	if err := client.AttachVolume(pool, volumeName, instanceName, deviceName, path); err != nil {
...
func DetachAndDeleteVolume(client IncusClient, pool, volumeName, instanceName, deviceName string) error {
	if err := client.DetachVolume(instanceName, deviceName); err != nil {
